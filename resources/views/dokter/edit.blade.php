...
<style>
    ...
    .info-box { background:#fffff0; border:1px solid #f6e05e; border-radius:8px; padding:.75rem 1rem; font-size:.82rem; color:#744210; margin-bottom:1rem; }
</style>
@endpush
...
<div class="card" style="max-width:800px">
    ...
    <div class="info-box">⚠️ Kosongkan field password jika tidak ingin mengubah password.</div>

    <form method="POST" action="{{ route('dokter.update', $dokter) }}">
        @csrf
        @method('PUT')

        <p class="section-title">Akun Login Dokter</p>
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                @error('email')<p class="invalid-msg">{{ $message }}</p>@enderror
            </div>
            <div class="form-group">
                <label>No. HP</label>
            ...
            <div class="form-group">
                ...
                @error('password')<p class="invalid-msg">{{ $message }}</p>@enderror
            </div>
            <div class="form-group">
        ...
        <p class="section-title">Data Medis Dokter</p>
        <div class="form-grid">
            <div class="form-group">
                <label class="required">Poli / Departemen</label>
                <select name="department_id" class="{{ $errors->has('department_id') ? 'is-invalid' : '' }}">
                    @foreach($departments as $dept)
                        <option value="{{ $dept->id }}" {{ old('department_id', $dokter->department_id) == $dept->id ? 'selected' : '' }}>
                            {{ $dept->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required">No. STR / SIP</label>
                <input type="text" name="license_number" value="{{ old('license_number', $dokter->license_number) }}"
                    class="{{ $errors->has('license_number') ? 'is-invalid' : '' }}">
                @error('license_number')<p class="invalid-msg">{{ $message }}</p>@enderror
            </div>
            <div class="form-group">
                <label class="required">Biaya Konsultasi (Rp)</label>
                <input type="number" name="consultation_fee" value="{{ old('consultation_fee', $dokter->consultation_fee) }}"
                    min="0" step="1000">
            </div>
        </div>

        <div class="form-actions">
            <a href="{{ route('dokter.show', $dokter) }}" class="btn btn-secondary">Batal</a>
